...
func (verb *stateVerb) Run(ctx context.Context, flags flag.FlagSet) error {
	...
	if len(verb.At) == 0 {
		...
		verb.At = append(verb.At, int(boxedCapture.(*service.Capture).NumCommands)-1)
	}
	at := make([]uint64, len(verb.At))
	for i, v := range verb.At {
		at[i] = uint64(v)
	}

	boxedTree, err := client.Get(ctx, c.Command(at[0], at[1:]...).StateTreeAfter().Path())
	if err != nil {
